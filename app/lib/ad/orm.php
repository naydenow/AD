...
// Adapted Naydenov Pavel

abstract class ad_orm
{
    protected static
        $conn,
        $database,
    ...
    {
        if (!self::$conn){
            self::$conn = AD::DI('ad_safemysql')->getMySQLi();
            self::$database = AD::config('db')['db'];
        }
     ...
     * @return mixed
     */
    public static function sql ($sql, $return = ad_orm::FETCH_MANY)
    {
        // shortcuts
    ...
    {
        $count = self::sql($sql, ad_orm::FETCH_ONE);
    ...
    {
        self::sql('TRUNCATE :database.:table', ad_orm::FETCH_NONE);
    }
    ...
    public static function pagination($start = 0,$limit = 50){
        return self::sql("SELECT * FROM :database.:table ".self::genPag($start,$limit));
    }

    public static function genPag($start = 0,$limit = 50){
    ...
    public static function retrieveByPK ($pk)
    {
        ...
        return $reflectionObj->newInstanceArgs(array($pk, ad_orm::LOAD_BY_PK));
    }
    ...
    public static function hydrate ($data)
    {
        ...
        return $reflectionObj->newInstanceArgs(array($data, ad_orm::LOAD_BY_ARRAY));
    }
     ...
     * @return mixed
     */
    public static function retrieveByField ($field, $value, $return = ad_orm::FETCH_MANY)
    {
        if (!is_string($field))
